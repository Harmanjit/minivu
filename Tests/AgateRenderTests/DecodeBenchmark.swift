...
/// Timings on real photos. Runs only when AGATE_BENCH_DIR points at a
/// folder of images, e.g. AGATE_BENCH_DIR=~/latent/TestAssets swift test --filter Benchmark
@Suite struct DecodeBenchmark {
    static let folder = ProcessInfo.processInfo.environment["AGATE_BENCH_DIR"].map { URL(fileURLWithPath: $0) }
