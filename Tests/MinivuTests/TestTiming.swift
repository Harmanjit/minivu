...
extension TestTiming {
    /// Polls `condition` until it holds or the patience runs out. The one
    /// wait the app's tests use: it captures main-actor state, so it stays
    /// on the main actor with the suites that call it.
    @MainActor static func waitUntil(seconds: Double = 10, _ condition: () -> Bool) async {
