...
///
///     AGATE_SNAPSHOT=/tmp/agate.png          turns the harness on; the PNG to write
///     AGATE_OPEN=~/Pictures/Trip             opened as if from Finder
///     AGATE_WINDOW_SIZE=1400x900             content size in points
///     AGATE_ACTIONS="openInViewer:;zoomIn:"  sent down the responder chain, 0.4 s apart
///     AGATE_SNAPSHOT_DELAY=2                 seconds to wait before capturing (default 1.5)
///     AGATE_SNAPSHOT_WINDOW=settings         capture the Settings window instead
///
...
enum SnapshotHarness {
    ...
    nonisolated struct Configuration: Equatable, Sendable {
        ...
        /// nil unless AGATE_SNAPSHOT is set.
        init?(environment: [String: String]) {
            guard let output = environment["AGATE_SNAPSHOT"], !output.isEmpty else { return nil }
            self.output = URL(fileURLWithPath: (output as NSString).expandingTildeInPath)
            if let path = environment["AGATE_OPEN"], !path.isEmpty {
                open = URL(fileURLWithPath: (path as NSString).expandingTildeInPath)
            }
            windowSize = environment["AGATE_WINDOW_SIZE"].flatMap(Self.parseSize)
            actions = environment["AGATE_ACTIONS"].map(Self.parseActions) ?? []
            if let text = environment["AGATE_SNAPSHOT_DELAY"], let seconds = Double(text), seconds >= 0 {
                delay = seconds
            }
            capturesSettings = environment["AGATE_SNAPSHOT_WINDOW"]?.lowercased() == "settings"
        }
    ...
    static func startIfRequested(app: AppDelegate,
        ...
        DispatchQueue.global(qos: .utility).asyncAfter(deadline: .now() + timeout) {
            FileHandle.standardError.write(Data("AGATE_SNAPSHOT: gave up after \(Int(timeout)) s\n".utf8))
            exit(2)
        }
    ...
    private static func report(_ message: String) {
        FileHandle.standardError.write(Data("AGATE_SNAPSHOT: \(message)\n".utf8))
    }
